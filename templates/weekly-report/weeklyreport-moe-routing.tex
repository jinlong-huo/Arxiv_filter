...
\begin{itemize}
  ...
  \item 分析训练中的负载均衡/熵/共激活等metrics
\end{itemize}


\subsection*{1.2\quad OCS 预触发逻辑设计}
如之前的讨论和RP中的内容,如果为ocs提供moe的专家偏好,首先要捕获模式,并进行预配置和CCL相关匹配.所以,这周的任务主要在基于已有模型权重后的再训练,验证训练前后的路由分布和专家偏好情况.

\textbf{Pipeline思路/逻辑：} LoRA 训练 捕获路由, 构建协同激活图, OCS 预配置表 推理时 GPU 间电路预连线.
